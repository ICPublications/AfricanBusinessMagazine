...
function buildSubmenu($category_id, $num_of_posts) {
	...
	get_term_children( $category_id, )

	if( category_has_parent($category_id) ){
...
function buildSubmenu($category_id, $category_name) {
	...
	if ( $query->have_posts() ) : 
		$child_categories = get_term_children( $category_id, 'category');
			...
			echo '<ul class="sub-menu" style="text-align:left">';
			
				if($category_id=='1061'){
    				
    					$args= array(
    						'post_type' => 'partnering-companies',
    						'orderby' => 'date'
    					);
    					$posts_array = get_posts($args);

    					foreach($posts_array as $post) : setup_postdata($post);
							echo '<li><a href="' . get_field( 'focus_link', $post->ID ) . '">' . get_the_title($post->ID) . '</a></li>';
						endforeach;
...
function banner_shortcode($atts) {
	...
	return '<div class="ad-unit '.$size.' align-'.$align.'">' . 
		...
		do_shortcode('[dfads params="groups='.$group.'&callback_function=customBannerOutput"]') . 
	'</div>';
}

...
require(['jquery', 'app/Contentslider', 'app/utils'], function($, Contentslider, utils) {
	...
	$(document).ready(function() {
		...
		if ( $('ul#promo-slider').length ) {
			...
			function homePrevSliderFunc() {
				// Highlighting for current title nav
				var prevTitleLink;

				if(titleLinks.filter('.is-current-title').parent().is(':first-child')) {
					prevTitleLink = titleLinks.last();
				} else {
					prevTitleLink = titleLinks.filter('.is-current-title').parent().prev().find('a');
				}
				
				titleLinks.removeClass('is-current-title');
				homeSlider.slideToPrev();
				prevTitleLink.addClass('is-current-title');
			}
			...
			$('#promo-slider').on( "swiperight", function( event ) { 
				homePrevSliderFunc();
			})

			$('#promo-slider').on( "swipeleft", function( event ) { 
				homeAutoSliderFunc();
			}) 
...
require(['jquery'], function($) {
	...
	if($('.leaderboard #video-banner-wrapper').length){
		$('.leaderboard').css('width', '1020px');
		$('.secondary').css('margin-top', '100px');

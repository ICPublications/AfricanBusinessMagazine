<?php 
	...
	//get_template_part('parts/_company-objects'); 
...
//
	...
	$crisks = new company('10766', 'Control Risks');
	$cameroon = new company('11995', 'Cameroon');
	$solutions = new company('13897' , 'Solutions and Co');
	$topafricanbrands = new company('18235' , 'Top African Brands');


	$companies = array($ticad , $ecobank, $afdb, $wbank, $ge, $crisks, $cameroon, $enneray, $solutions, $topafricanbrands);
	...
	foreach($companies as $company){
		if($company->name == $category->name){
			...
			$linkedin_link = $company->youtube_link;
		...
		<?php 
			if( $parent_name == 'In Focus'){
				//echo $category_post->ID;
					...
					echo '<a class="" href="'. $company_url.'"><img src="'.  $banner_img .'" style="width:100%; float: left; '; if( $name == 'Solutions and Co' ){ echo 'margin-bottom:40px; margin-top:30px'; } echo '" /></a>';
					
					if($press_release_link!="" && $name!='Control Risks'){ 
...
“I guess I would say that if you are from a small country, purely from the measure of shareholding you’re fighting an uphill battle. On the other hand, the current president is one of those small country candidates. I still think it’s anyone’s guess how this turns out.”</p>
		...
		<?php 
			...
			if( $parent_name == 'In Focus'){ 
				//echo $category_post->ID;
				if( $name == "Solutions and Co" ){
			   		echo '<iframe width="100%" height="1126px" src="https://s3-eu-west-1.amazonaws.com/soco-2016/index.html" frameborder="0" scrolling="no"></iframe>';
			   		echo '<h2 style="text-align: center; color: #ec8a1e;"" >Towards Sustainable E-book</h2>';
			   		echo '<iframe width="100%" height="600" src="//e.issuu.com/embed.html#26662905/40300904" frameborder="0" allowfullscreen></iframe>';
				}else{
		...
		<?php 
			if( $parent_name == 'In Focus'){ 
			?>
				<div id="sidebar" class="l-4-9" style="padding-top:4px">
					<?php if($name=='General Electric'){ ?>
						<div>
							
							<h3 style="border-bottom: 1px solid #025090; color: #025090;">Dangote Cement plant in Obajana, Nigeria</h4>
							<div id="ytplayer"></div>
					 ...
					 <?php if($name=='ticad'){ ?>
						<div>
							...
							<h3 style="border-bottom: 1px solid #025090; color: #025090;">TICAD Video</h4>
							<div id="ytplayer"></div>
							...
							<script>
							  ...
							  function onYouTubePlayerAPIReady() {
							    player = new YT.Player('ytplayer', {
							      height: '200',
							      width: '210',
							      //playerVars: { 'autoplay': 1, 'controls': 1, 'playlist':['yPrEjODmKhs', 'kIph8wsEBjU', 'fAynSnvMw6Y', 'fTJtLISse6E', 'QMtmRO9imL4' ]},
							      //playerVars: { 'autoplay': 1, 'controls': 1, 'playlist':'PLDyns77mBfK8re9Ybu-JwVsoIOOQpON8K'},
							      playerVars: 
						          {
						            listType:'playlist',
						            list: 'PLDyns77mBfK8re9Ybu-JwVsoIOOQpON8K'
						          },
							      events: {
							        'onReady': onPlayerReady
							      }
					...
					<div id="company-bio" style="background-color: grey; color: white; font-family:'futura-pt',sans-serif; padding:11px;">
					 ...
					 <?php if($name=='Control Risks'){ ?>
						<div>
							<?php  echo '<h3 style="border-bottom: 1px solid ' . $company_col . '; color: '. $company_col .'">Risk Map 2015</h3>'; ?>
							<a target="_blank" href="http://africanbusinessmagazine.com/uncategorised/control-risks-risk-map-2015-infographic/" /><img src="http://africanbusinessmagazine.com/wordpress/wp-content/uploads/2015/11/RiskMap.png" /></a>
